...
public class MonsterSpawner {
    ...
    private static EntityType<? extends Mob> getMobTypeForTier(int tier) {
        // Tier 1-3: Early game (Depths 1-5) - Weak wasteland scavengers
        if (tier <= 3) {
        ...
        // Tier 4-6: Mid game (Depths 6-10) - Dangerous mutants
        else if (tier <= 6) {
            EntityType<?>[] midMobs = {
        ...
        // Tier 7-9: Late game (Depths 11+) - Elite wasteland horrors
        else {
            EntityType<?>[] lateMobs = {
    ...
    public static String getMobDescriptionForTier(int tier) {
        if (tier <= 3) {
            return "EARLY tier (Zombies, Skeletons, Spiders)";
        } else if (tier <= 6) {
            return "MID tier (Creepers, Witches, Phantoms)";
        } else {
            return "LATE tier (Wither Skeletons, Endermen, Blazes)";
        }
    }
}
